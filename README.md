...
video player works once you allow it.** Chrome only. See [Browsers](#browsers).

<a id="browsers"></a>

## Browsers

**Chrome.** That is the supported browser, and the only one this is tested and
verified on.

Other Chromium browsers are a maybe rather than a promise. The extension is
Manifest V3 and nothing in it is Chrome-specific, but the room panel is not
decoration: the socket lives in it, so a browser that cannot open one cannot
chat, cannot show members, and cannot sync. Chrome puts it in the native side
panel. Where `chrome.sidePanel` is missing or inert, the panel falls back into
the page as an extension iframe in a shadow root, then to a small window if the
page cannot host one.

That fallback is built and covered by tests, and it is why the code is shaped the
way it is:

- **A resolved call is not an opened panel.** Some browsers ship the API, resolve
  `sidePanel.open()` and paint nothing, so a panel counts only once its document
  actually exists (`runtime.getContexts`), never because a call came back clean.
...
- **Navigation destroys an in-page panel.** Clicking through to the next episode
  would take the socket with it, so a fresh document asks the worker whether it
  was the one hosting and gets it back. The dropped port a navigation causes is
  not read as "the panel was closed", which would otherwise end the room every
  time somebody changed the page.

Tested, but not a support claim. Browsers that skin or fork Chromium can differ
in ways no test here would catch, and several cannot be automated at all, so the
suite reproduces the *condition* (a side panel API that answers and opens
nothing) inside a real Chrome rather than pretending to drive another browser.
If it works for you elsewhere, good. If it does not, that is expected, and Chrome
is the answer.
