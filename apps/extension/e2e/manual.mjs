...
// fake sessions so both browsers skip the login gate. the token is signed with the
// server's own JWT_SECRET so joinRequiresLogin() sees a real user; without the
// secret it degrades to an unverifiable token (guest, the server may deny rooms)
...
async function launch(x, n) {
  ...
  // the first install opens the onboarding tab before the fake session lands; close it
  await new Promise((r) => setTimeout(r, 500));
  for (const p of ctx.pages()) {
    if (p.url().includes('src/onboarding')) await p.close().catch(() => {});
  }
  return ctx;
}
...
const contexts = [await launch(0, 1), await launch(780, 2)];
