...
function runTop(): void {
  ...
  document.addEventListener('fullscreenchange', () => {
    reparentPanel();
    const note = document.getElementById('ab-love');
    if (note) {
      const noteHost = reactionHost();
      if (note.parentElement !== noteHost) noteHost.appendChild(note);
    }
    const layer = document.getElementById('ab-reactions');
    if (!layer) return;
  ...
  chrome.runtime.onMessage.addListener((msg: TabMessage, sender, sendResponse) => {
    ...
    if (msg.type === 'OPEN_PANEL') {
      // every frame gets this; only the top one has a viewport worth mounting into
  ...
  function stopSync() {
    ...
    stopTargetWatch();
    stopNavWatch();
    document.getElementById('ab-reactions')?.remove();
    document.getElementById('ab-love')?.remove();
    hidePanel();
  }
  ...
  // the relay echoes a reaction to its sender too, so both rooms tally the same
  // hearts and cross fifteen on the same one
  const love = createLoveCounter(() => showLoveNote(reactionHost()));

  function spawnReaction(emoji: string): void {
    love.tally(emoji);
    let layer = document.getElementById('ab-reactions');
    if (!layer) {
      layer = document.createElement('div');
      layer.id = 'ab-reactions';
